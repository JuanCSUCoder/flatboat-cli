...
pub fn map2rs(result_tuple: Bound<'_, PyAny>) -> Result<(String, Vec<String>), PyErr> {
    ...
    debug!("Extracting values from PyAny...({:?},{:?})", first, second);
    let first_str: String = first.extract()?;
    let second_str: Vec<String> = second.extract()?;
